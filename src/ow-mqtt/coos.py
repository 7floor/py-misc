...
class Task(object):
    ...
    def run(self):
        while True:
            try:
                #print self.target.__name__
                result = self.target.send(self.sendval)
                if isinstance(result, SystemCall):
                    return result
                if isinstance(result, types.GeneratorType):  # call
                    self.stack.append(self.target)
                    self.sendval = None
                    self.target = result
                elif result is not None:  # exit from function returning value
                    if not self.stack:
                        return
                    self.sendval = result
                    ...
                    return
            except StopIteration:  # exit from procedure
                if not self.stack:
                    raise
                self.sendval = None
                self.target = self.stack.pop()
